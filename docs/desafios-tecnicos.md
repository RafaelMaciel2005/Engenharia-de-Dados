...
**Mitigação inicial:** o framework de validação foi mantido 100% em funções nativas da DataFrame API (inclusive o checksum, que usa `xxhash64` nativo em vez de UDF Python justamente por isso), e o teste sintético passou a gerar dados via CSV + `spark.read` (mesmo caminho do código de produção). Isso contornou o sintoma, mas o risco seguia latente: qualquer código futuro com UDF Python, `toPandas()` ou `createDataFrame` local esbarraria nele — **no cluster**.

**Correção definitiva (aplicada):** a raiz do problema era a imagem do Spark. A base anterior era `eclipse-temurin:17-jre-jammy`, e o `python3` do Ubuntu 22.04 é o 3.10 — enquanto o driver (Airflow) roda 3.11. A imagem do Spark foi reescrita para basear em `python:3.11-slim-bookworm` (mesmo *minor* do driver, `python3` nativamente 3.11) com `openjdk-17-jre-headless` — simétrica com o Dockerfile do Airflow. Para não depender do `archive.apache.org` (que serve o tarball do Spark a ~180 KB/s daqui e chegava a pendurar o build), a distribuição do Spark passou a vir da imagem **oficial `apache/spark:3.5.1`** via multi-stage `COPY`, restando só os JARs do S3A vindos do Maven Central.

O **mesmo tratamento foi aplicado à imagem do Airflow**: ela também precisa da distribuição completa do Spark (o `SparkSubmitOperator` chama o binário `spark-submit` de verdade — o `pyspark` do pip só cobre a API Python, não o launcher) e também baixava o tarball do `archive.apache.org`. Agora puxa o Spark da mesma imagem oficial via multi-stage. As duas imagens ficaram consistentes: build rápido, reproduzível e sem dependência do mirror lento. Efeito colateral bônus: o download do JAR do S3A caiu de ~35 min para ~20 s (Maven Central serve a ~14 MB/s, contra os ~180 KB/s do archive).

**Validação:** com driver e executor agora em 3.11, o caminho exato que falhava — `createDataFrame` a partir de uma lista local, executado no cluster (`--master spark://spark-master:7077`) — passou a rodar sem erro (driver 3.11.12 × executor 3.11.15, mesmo minor). Um smoke test de uma task real da Bronze (leitura/escrita S3A + validação + manifest) confirmou que a imagem nova segue 100% funcional.

**Lição:** um cluster pode estar "funcionando" e ainda assim ter incompatibilidades latentes que só certos caminhos de código exercitam. Testes negativos não validam só as regras — expõem a infraestrutura. E vale rastrear o defeito até a raiz (a imagem base), não só contornar o sintoma. Os testes unitários (`tests/`) sempre usaram `createDataFrame` sem problema porque rodam em Spark `local[1]`, onde driver e executor compartilham o mesmo interpretador (ver [CI/CD](ci-cd.md)).
